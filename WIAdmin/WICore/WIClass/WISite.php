...
class WISite 
{
	...
	public function lang_Settings($settings) 
	{
		...
		public function lang_Selector($settings, $radioValue) 
	{
			...
			$user_id = 1;
				...
				$query = $this->WIdb->prepare('UPDATE `wi_site` SET  `multi_lang` =:multilanguage, `lang_choice`=:lang_choice  WHERE `id` = :user_id');
				$query->bindParam(':multilanguage', $lang, PDO::PARAM_STR);
				$query->bindParam(':lang_choice', $selector, PDO::PARAM_STR);
				$query->bindParam(':user_id', $user_id, PDO::PARAM_INT);
				$query->execute();
